...
export function NabvarMobile() {
  ...
  return (
    <div
      className="fixed -bottom-[0px] z-40 w-full h-[52px] bg-zinc-100 dark:bg-[#111316] justify-between items-center flex px-4
    ...
    >
      ...
      {session?.user.role === "ADMIN" && (
        <Link
          ...
          } h-10 w-10 flex flex-col justify-center items-center 0 ease-in-out duration-500 hover:bg-zinc-300/50 dark:hover:bg-zinc-700/50 rounded-2xl`}
        >
      ...
      {status === "loading" ? (
        ...
        status === "authenticated" && <ProfileIcon user={session.user} mobile={true} />
      )}
    </div>
  )
}
